...
const Sliders = () => {
    ...
    const fetchData = async (p0: number) => {
        try {
            setLoading(true);
            const res = await apiInstance.get(`/slider`);
            if (res) {
    ...
    return (
        <div>
            ...
            <div className="col-span-12 xl:col-span-8">
                <SliderTable fetchData={fetchData} heading={"List of Sliders"} tableData={tableData} loading={loading} />
            </div>
        </div>
    )
}
