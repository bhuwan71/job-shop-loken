...
const SliderTable: React.FC<TableOneProps> = ({
  heading,
  tableData,
  loading,
  fetchData,
  ...
  const handleDelete = async (rowData: any) => {
    try {
      const response = await apiInstance.delete(`/slider/${rowData.id}`);
      if (response) {
  ...
  useEffect(() => {
    const filtered = tableData?.filter((rowData: { title: string }) =>
      rowData.title?.toLowerCase().includes(searchTerm.toLowerCase()),
    );
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="block p-2 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-full sm:w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-blue-500 dark:focus:border-blue-500"
                placeholder="Search for Sliders"
                  ...
                  {filteredData.map(
                    (rowData: any, index: Key | null | undefined) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">{rowData.title}</td>
